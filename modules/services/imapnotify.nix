    ...
    in {
      ...
      value = {
        ...
        Service = {
          ...
          Type = "simple";
          Environment = [
            (lib.mkIf account.notmuch.enable "NOTMUCH_CONFIG=${config.xdg.configHome}/notmuch/default/config")
            "PATH=${cfg.path}"
          ];
        };
...
in {
  ...
  options = {
    services.imapnotify = {
      ...
      path = mkOption {
        description = "List of packages to provide in PATH for the imapnotify service";
